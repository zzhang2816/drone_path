...
def take_off(p1,p2,p3):
    ...
    startY=2
    # for the rightmost
    p3.append(Bezier_up(startPos=[3,startY,startHeight], endPos=[3.4,0.6,2],time=5*t))
    # for the middle
    p2_start = [2,startY,startHeight]
    p2.append(Bezier_up(startPos=p2_start, endPos=[2,4,2],time=5*t))
    # for the leftmost
    p1_start = [1,startY,startHeight]
    p1.append(Bezier_up(startPos=p1_start, endPos=[0.6,0.6,2],time=5*t))
    return p1, p2, p3
...
def roundabout(p1,p2,p3):
    # 2
    p2_start=p2[-1][-1]
    c2=Circle(startPoint=p2_start,center=[1.5,2,p2_start[2]],time=4*t)
    rb2=c2.getLinePoints(4.8*math.pi)
    # 1, 3
    rb1=rb2.copy()
    rb1[:,0]=rb1[:,0]+0.6
    rb3=rb2.copy()
...
if __name__ == '__main__':
    ...
    p1,p2,p3=toward_people(p1, p2, p3)

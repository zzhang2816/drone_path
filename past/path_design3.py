...
def take_off(p1,p2,p3):
    ...
    p1.append(Bezier_up(startPos=p1_start, endPos=[3.4,3.4,2],time=5*t))
    return p1, p2, p3
...
def circle(p1,p2,p3):
    ...
    p3.append(c3.getLinePoints(6.5*math.pi))
    # for the second
    p2_start=p2[-1][-1]
    c2=Circle(startPoint=p2_start,center=[2,2,p2_start[2]],time=20*t)
    p2.append(c2.getLinePoints(5*math.pi))
    # for the third
    p1_start=p1[-1][-1]
...
def roundabout(p1,p2,p3):
    ...
    p3.append(c3.getLinePoints(4*math.pi))
    # for the second
    p2_start=p2[-1][-1]
    c2=Circle(startPoint=p2_start,center=[2,2,p2_start[2]],time=16*t)
    p2.append(c2.getLinePoints(4*math.pi))
    p2.append(Stop(p2_start,0.5*t))
    # for the third
    p1_start=p1[-1][-1]
    c1=Circle(startPoint=p1_start,center=[2,2,p1_start[2]],time=16*t)
    p1.append(c1.getLinePoints(4*math.pi))
    p1.append(Stop(p1_start,t))
    return p1, p2, p3 
